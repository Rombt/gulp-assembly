...
function rmbt_get_redux_field( $id_field, $kses = false, $all_tags_allowed = false ) {
	global $rmbt_premium_theme_1k_options;

	if ( ! array_key_exists( $id_field, $rmbt_premium_theme_1k_options ) )
		return "";

	if ( $kses ) {
		return class_exists( 'ReduxFramework' ) ? wp_kses( $rmbt_premium_theme_1k_options[ $id_field ], 'post' ) : "";
	} elseif ( $all_tags_allowed ) {
		return class_exists( 'ReduxFramework' ) ? $rmbt_premium_theme_1k_options[ $id_field ] : "";
	}
	return class_exists( 'ReduxFramework' ) ? esc_html__( $rmbt_premium_theme_1k_options[ $id_field ] ) : "";
}
